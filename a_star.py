...
class BestFirst:
    ...
    def __init__(self, graph, h_fn, solution_fn, g_fn=lambda s: 1):
        """
    ...
    def next(self):
        ...
        for cn in next_node.get_children():  # 5
            if hasattr(cn, 'g'):  # 6
                ...
                cn.prev = next_node           # |
                cn.g = next_node.g + self.g_fn(cn.state)          # |
    ...
    def update_g(self, node, preceding_node):
        ...
        :return: None
        """
        if node.g > preceding_node.g + self.g_fn(node.state):
            node.g = preceding_node.g + self.g_fn(node.state)
